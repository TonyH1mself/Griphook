...
**Indexes / RLS:** Dashboard and list views use `entries(created_by_user_id, occurred_at)` and `entries(bucket_id, occurred_at)` (see `init` + `hardening` migrations). `bucket_members` updates are restricted to **admins** by existing RLS; member share editing in the app relies on that policy.
