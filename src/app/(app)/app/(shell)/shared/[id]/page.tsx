...
export default async function SharedBucketDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const totalExpenses =
    entries?.filter((e) => e.transaction_type === "expense").reduce((s, e) => s + Number(e.amount), 0) ?? 0;

  return (
    <div className="space-y-8">
      <div>
        <Link href="/app/shared" className="text-sm font-medium text-slate-500 hover:text-slate-800 dark:hover:text-slate-200">
          ← Shared
        </Link>
      ...
      <Card>
        ...
        <CardDescription>
          Soll uses each member’s share of total expenses; Ist is what they actually logged as payer this month.
        </CardDescription>
          ...
          <table className="w-full min-w-[520px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {breakdown.map((row) => {
                ...
                return (
                  <tr key={row.userId}>
                    <td className="py-3 pr-4 font-medium text-slate-900 dark:text-white">{label}</td>
                    <td className="py-3 pr-4 tabular-nums text-slate-600 dark:text-slate-300">
                      {row.sharePercent.toFixed(1)}%
                    </td>
                    <td className="py-3 pr-4 tabular-nums text-slate-700 dark:text-slate-200">{formatEur(row.shareAmount)}</td>
                    <td className="py-3 pr-4 tabular-nums text-slate-700 dark:text-slate-200">{formatEur(row.actualAmount)}</td>
                    <td className="py-3 tabular-nums text-slate-900 dark:text-white">{formatEur(row.delta)}</td>
                  </tr>
                );
              })}
            </tbody>
